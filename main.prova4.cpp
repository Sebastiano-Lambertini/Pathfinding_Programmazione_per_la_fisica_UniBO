...
std::vector<Path> final_aggiratore(Point A, Point B, const std::vector<std::vector<bool>> &griglia) {
Grid grid(griglia);
...
std::vector<int> trova_id_vietati_pieni(
...
std::vector<Point> estrai_coordinate(const std::string &linea,
  ...
  while (pos < stringa_coordinate.size()) {
    size_t end =
        stringa_coordinate.find(';', pos);
    if (end == std::string::npos) {
      end = stringa_coordinate.size();
    }

    std::string punto_str = stringa_coordinate.substr(pos, end - pos);

    if (!punto_str.empty()) {
      size_t spazio = punto_str.find(' ');
      if (spazio != std::string::npos) {

        double x = std::stod(punto_str.substr(0, spazio));

        size_t y_start = spazio + 1;
        while (y_start < punto_str.size() && punto_str[y_start] == ' ') {
          ++y_start;
        }

        size_t y_end = y_start;
        while (y_end < punto_str.size() &&
               (isdigit(punto_str[y_end]) || punto_str[y_end] == '-' ||
                punto_str[y_end] == '.')) {
          ++y_end;
        }

        double y = std::stod(punto_str.substr(y_start, y_end - y_start));

        punti.push_back(
            {static_cast<int>(x * fattore_scala),
             static_cast<int>(y * fattore_scala)});
      }
    }

    size_t nuova_pos = salta_spazi(stringa_coordinate, end + 1);
    if (nuova_pos == pos) {
      break;
    }
    pos = nuova_pos;
...
void oggetti_to_griglia(const oggetto_poligonale_vietato &oggetto,
  ...
  for (const auto &punto : oggetto.contorno) {
    int gx{0};
    int gy{0};
    mondo_a_griglia(static_cast<double>(punto.x), static_cast<double>(punto.y), min_x, max_x, min_y, max_y,
                    larghezza_griglia, altezza_griglia, gx, gy);
...
void esporta_percorso_omap(
    ...
    const std::string &filename_output, const std::vector<Path> &paths,
    const std::vector<id_to_symbol> &ids_symbols, double min_x, double max_x,
    double min_y, double max_y, int larghezza_griglia, int altezza_griglia,
  ...
  while (std::getline(in, linea)) {
    ...
    if (!oggetto_aggiunto && linea.find("</objects>") != std::string::npos ) {
      ...
      for (size_t i = 0; i < percorso.size(); ++i) {
        double x_mondo =
            min_x + (percorso[i].x + 0.5) * (max_x - min_x) / larghezza_griglia;
        double y_mondo =
...
int main() {
  ...
  calcola_dimensioni_max_min(oggetti_vietati, min_x, max_x, min_y, max_y, arrivo_mondo, partenza_mondo);
  int risoluzione = 500;
  double larghezza_mondo = max_x - min_x;
  double altezza_mondo = max_y - min_y;
...
//per smoothare linee dopo si può fare che prendo punti di passaggio, converto quelli e li metto dentro