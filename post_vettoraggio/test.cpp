...
TEST_CASE("filtra_percorsi: tutti i percorsi entro 1.2x il costo minimo") {
  ...
  for (int x = 2; x <= 12; ++x) {
    ...
    {
      griglia.rendi_non_oltrepassabile(x, 7);
    }
  }
...
TEST_CASE("Griglia::e_oltrepassabile: celle libere e occupate") {
  Griglia griglia(5, 5);
  griglia.rendi_non_oltrepassabile(3, 2); // occupa (x=3, y=2)
...
TEST_CASE("Griglia::controlla_id_ostacolo: blocco connesso ha lo stesso id") {
  ...
  for (int y = 1; y <= 3; ++y) {
    for (int x = 2; x <= 4; ++x) {
      griglia.rendi_non_oltrepassabile(x, y); //ho scambiato x e y, vedere se va il test
    }
  }
...
TEST_CASE("Filtra_percorsi esclude percorsi troppo lunghi") {
  ...
  // punti fittizi per sicurezza(assert su percorsi vuoti)
...
TEST_CASE("vede_punto_di_arrivo: ostacolo sulla linea ritorna false") {
  ...
  griglia.rendi_non_oltrepassabile(5,5);

  Punto P(0, 0);
  Punto B(9, 9);
  CHECK(e_libero_fino_a_punto_di_arrivo(P, B, griglia) == false);
}

TEST_CASE("vede_punto_di_arrivo: ostacolo leggermente spostato non blocca") {
  Griglia griglia(10, 10);

  griglia.rendi_non_oltrepassabile(5,6);
