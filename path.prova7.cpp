...
Point ultimo_punto_prima_del_primo_ostacolo(Point A, Point B,
                                            const Grid &griglia) {
  std::vector<Point> linea = linea_dritta(A, B);
...
// dell'arrivo(B)(non entra in loop)
bool vede_spazio_in_direzione_arrivo(Point P, Point B, const Grid &griglia,
                                     int id_ostacolo) {
  std::vector<Point> linea = linea_dritta(P, B);
  for (size_t i = 1; i < linea.size(); ++i) {
    Point p = linea[i];
...
Border_exit segui_il_bordo_monodirezione(Point partenza, Point destinazione,
  ...
  // gestisce casi limite con contatto in diagonale su ostacolo
  // ////////////////////////////////////////////////////////////in teoria
  ...
  for (int d = 0; d < 4; ++d) {
    ...
    if (!griglia.is_free(vicino) &&
      ...
      if (direzione == orario) {
        dir_precedente = (d + 3) % 4;
      } else {
        dir_precedente = (d + 1) % 4;
      }
      break;
    }
  }
  // se non si sposta dir => i vicini sono liberi,  ricontrolla e va a distanza
  // 2
  if (dir_precedente == -1) {
    bool spostato = false;
    for (int d = 0; d < 4; ++d) {
      ...
      for (int k = 0; k < 4; ++k) {
        ...
        if (!griglia.is_free(adiacente) &&
          ...
          percorso.points.push_back(corrente);
          if (direzione == orario) {
            dir_precedente = (k + 3) % 4;
          } else {
            dir_precedente = (k + 1) % 4;
          }
          spostato = true;
          break;
        }
  ...
  while (passi < MAX_PASSI) {
    if (vede_spazio_in_direzione_arrivo(corrente, destinazione, griglia,
                                        id_ostacolo)) {
    ...
    int priorita[4]; // controlla prima verso bordo, poi dritto, poi opposto,
                     // poi torna indietro;   inserisce in prorita che poi viene
                     // usato dopo per decidere cosa porvare prima
    if (direzione == orario) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (visitati.find(prossimo) != visitati.end()) {
        continue;
      }
...
void costruisci_percorsi(Point corrente, Point destinazione, Point origine,
                         ...
                         int costo_corrente, std::vector<Path> &percorsi_output,
                         int profondita) {
  ...
  if (vede_punto_di_arrivo(corrente, destinazione, griglia)) {
    ...
    costo_corrente +=
        static_cast<int>(linea_dritta(corrente, destinazione).size());
    Path p;
  ...
  // rimosso controllo ostacoli già visitati: può riattraversare ostacoli già
  // incontrati
  ...
  if (!tratto_P0.empty() && tratto_P0.back() == P0) {
    tratto_P0.pop_back();
  }
  ...
  for (int d = 0; d < 2; ++d) {
    ...
    // C: uscita verso destinazione
    Border_exit uscita_dest =
        segui_il_bordo_monodirezione(P0, destinazione, griglia, id_ost, dir);
    ...
    // P: uscita verso origine
    Border_exit uscita_orig =
        segui_il_bordo_monodirezione(C, origine, griglia, id_ost, dir_inversa);
    if (!uscita_orig.exit.e_valido()) {
      continue;
    }
    ...
    if (nuovi_waypoints.empty()) {
      nuovi_waypoints.push_back(corrente);
    }
    if (!(P == corrente) &&
        std::find(nuovi_waypoints.begin(), nuovi_waypoints.end(), P) ==
            nuovi_waypoints.end()) {
      nuovi_waypoints.push_back(P);
    }
    if (!(C == P) && std::find(nuovi_waypoints.begin(), nuovi_waypoints.end(),
                               C) == nuovi_waypoints.end()) {
      nuovi_waypoints.push_back(C);
    }

    int nuovo_costo =
        costo_corrente + costo_verso_P0 + costo_bordo_C + costo_bordo_P;

    std::vector<std::pair<int, int>> nuovi_detours = detours_correnti;
    nuovi_detours.push_back({id_ost, d});

    costruisci_percorsi(C, destinazione, origine, griglia, nuovi_waypoints,
                        nuovi_detours, nuovo_costo, percorsi_output,
                        profondita + 1);
  }
}
...
void printa_path(const Path &p, int larghezza, int altezza,
  ...
  std::vector<std::vector<char>> visuale(altezza,
                                         std::vector<char>(larghezza, '.'));
  for (int y = 0; y < altezza; ++y)
    for (int x = 0; x < larghezza; ++x)
      if (!griglia[y][x]) {
        visuale[y][x] = '#';
  ...
  for (size_t i = 0; i < p.points.size() - 1; ++i) {
    ...
    for (const Point &pt : linea)
      if (pt.x >= 0 && pt.x < larghezza && pt.y >= 0 && pt.y < altezza &&
          visuale[pt.y][pt.x] != '#') {
        visuale[pt.y][pt.x] = '*';
      }
  }
  ...
  if (!p.points.empty()) {
    ...
    if (A.x >= 0 && A.x < larghezza && A.y >= 0 && A.y < altezza) {
      visuale[A.y][A.x] = 'A';
    }
    if (B.x >= 0 && B.x < larghezza && B.y >= 0 && B.y < altezza) {
      visuale[B.y][B.x] = 'B';
    }
  }
