...
Griglia::Griglia(const std::vector<std::vector<TipoCella>> &g) : griglia(g) {
  assert(!g.empty());
  assert(!g[0].empty());
...
void Griglia::rendi_non_oltrepassabile(int x, int y) {
  ...
  griglia[y][x] = TipoCella::non_oltrepassabile;
}
...
void Griglia::forma_griglia_vettore(int altezza_calcolata,
  ...
  griglia.assign(
      static_cast<size_t>(altezza_calcolata),
      std::vector<TipoCella>(static_cast<size_t>(larghezza_calcolata),
                             TipoCella::oltrepassabile));
  id_ostacolo.assign(altezza_calcolata,
                     std::vector<int>(larghezza_calcolata, -1));
}
...
void Griglia::riempi(int inizio_x, int inizio_y, int id) {
  ...
  id_ostacolo[static_cast<size_t>(inizio_y)][static_cast<size_t>(inizio_x)] =
      id;
  while (!q.empty()) {
    Punto p = q.front();
    q.pop();
...
auto nuova_griglia = griglia;
  for (int x = 0; x != larghezza; ++x) {
    for (int y = 0; y != altezza; ++y) {
      if (griglia[y][x] == TipoCella::non_oltrepassabile) // cella vietata
      {
        for (int dy = -1; dy != 2; ++dy) {
          ...
          for (int dx = -1; dx != 2; ++dx) {
            ...
            if (nx >= 0 && nx < larghezza && ny >= 0 && ny < altezza) {
              nuova_griglia[ny][nx] = TipoCella::non_oltrepassabile;
            }
          }
        }
      }
