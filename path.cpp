...
void Griglia::rendi_non_oltrepassabile(int x, int y) {
  ...
  griglia[y*larghezza+x] = TipoCella::non_oltrepassabile;
}
...
int Griglia::ottieni_id_ostacolo(int x, int y) const {
  ...
  return id_ostacolo[y*larghezza+x];
}
...
void Griglia::riempi(int inizio_x, int inizio_y, int id) {
  ...
  id_ostacolo[inizio_y*larghezza+inizio_x] =
      id;
  while (!q.empty()) {
    Punto p = q.front();
    q.pop();
    for (int d = 0; d != 4; ++d) {
      ...
      if (nx >= 0 && nx < larghezza && ny >= 0 && ny < altezza) {
        if (!e_oltrepassabile(nx, ny) &&
            id_ostacolo[ny*larghezza+nx] ==
                -1) {
          id_ostacolo[ny*larghezza+nx] = id;
          q.push(Punto(nx, ny));
        }
      }
    }
...
auto nuova_griglia = griglia;
  for (int x = 0; x != larghezza; ++x) {
    for (int y = 0; y != altezza; ++y) {
      if (griglia[y*larghezza+x] == TipoCella::non_oltrepassabile) // cella vietata
      {
        for (int dy = -1; dy != 2; ++dy) {
          ...
          for (int dx = -1; dx != 2; ++dx) {
            ...
            if (nx >= 0 && nx < larghezza && ny >= 0 && ny < altezza) {
              nuova_griglia[ny*larghezza+nx] = TipoCella::non_oltrepassabile;
            }
          }
        }
      }
